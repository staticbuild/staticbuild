...
function renderView(req, res, next) {
  ...
  if (viewpath.length === 0)
    viewpath = 'index';
  renderViewAt(viewpath, req, res, next);
}

function renderViewAt(viewpath, req, res, next) {
  res.render(viewpath + '.' + build.engine.jade.extension, function (err, html) {
    if (renderViewCompleted(err, html, viewpath, req, res, next))
      return;
    res.render(viewpath + '.' + build.engine.nunjucks.extension, function (err, html) {
      if (renderViewCompleted(err, html, viewpath, req, res, next))
        return;
      if (path.basename(viewpath) === 'index')
        next();
      else
        renderViewAt(viewpath + '/index', req, res, next);
    });
  });
}

function renderViewCompleted(err, html, viewpath, req, res, next) {
  if (!err) {
    // No error? Great. Send the html and we're done.
    res.send(html);
    return true;
  }
  // 404? Continue trying to render.
  var msg = err.toString();
  if (msg.substr(0, 25) === 'Error: template not found')
    return false;
  // 500. Done.
  next(err);
  return true;
