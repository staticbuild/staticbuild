...
function initViewEngine() {
  ...
  // There is no default view engine because app.render is not being used.
  // TODO: Add a setting for the default engine.
  app.set('view engine', build.engine.nunjucks.extension); //'');
...
function initNunjucks() {
  ...
  options.express = app;
  nunjucks_env = nunjucks.configure(relativePathToViews, options);
...
function createViewContext() {
  ...
  lodash.assign(context, build.globals.functions);
...
function findLocalsFiles(viewpath) {
  ...
  var pattern = String.prototype.replace.apply(build.localsfile, [
