...
function run() {
  ...
  .version('v', 'Show version number.', titleVersion())
  .alias('v', 'version')
...
function setup(yargs) {
  yargs = configureYargs(yargs)
  .usage(commandUsage('setup', '[options] [path]',
    'Setup.\n\n' + 
...
function parseDelay(value) {
  ...
  if (value.match(/^\d*ms$/)) {
    ...
    // Otherwise, parse for seconds, with or without time specifier, 
...
function parseDevserverRestart(args) {
  args.restartDelay = 0;
  // args.restart will be -1 if nodemon should be disabled.
  if (args.restart > -1) {
