...
function chdir(dirpath) {
  try {
    ...
    cli.printErrorAndExit('Error "' + err + '" changing to directory: ' + dirpath, 1005);
  }
  return false;
}
...
function run(opts) {
  ...
  var build = new StaticBuild(clargs.path, {
    bundlesEnabled: clargs.bundling,
  ...
  if (build.errors.length > 0) {
    ...
    cli.printErrorAndExit('Invalid staticbuild configuration: ' + build.filepath, 1006);
    return;
  }
