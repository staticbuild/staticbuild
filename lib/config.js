...
function load(opts) {
  ...
  var result = {};
  if (!staticbuild.requireFile(config.filepath, result))
    return false;
  if (!loadAll(config, result.obj))
    return false;
...
function loadDataFile(config, cfgdata) {
  ...
  if (staticbuild.requireFile(fullpath, result))
